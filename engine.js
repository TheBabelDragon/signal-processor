...
async function buildGraph(ctx, sourceBuffer, p, opts) {
  ...
  if (p.mode === 'binaural') {
    ...
    if (p.mode === 'isochronic') {
      ...
      const cspec = (p.carrier_hz && p.carrier_hz.type) ? p.carrier_hz
        : { type: 'constant', value: Number(p.carrier_hz) || 200 };
      scheduleParam(carrier.frequency, cspec, now, totalDuration);
      const mix = ctx.createGain();
      mix.gain.value = 1;
      src.connect(musicGain).connect(mix);
      carrier.connect(toneGain).connect(mix);
      mix.connect(worklet).connect(fadeGain);
      carrier.start(now);
      toneNodes = [worklet, carrier];
    }
    fadeGain.connect(masterGain).connect(limiter).connect(ctx.destination);
    src.start(now);
    return { src: src, limiter: limiter, masterGain: masterGain, fadeGain: fadeGain, toneNodes: toneNodes, startedAt: now, musicGain: musicGain, toneGain: toneGain };
  } else {
    src.connect(musicGain).connect(fadeGain);
  }
  ...
  return { src: src, limiter: limiter, masterGain: masterGain, fadeGain: fadeGain, toneNodes: toneNodes, startedAt: now, musicGain: musicGain };
}
